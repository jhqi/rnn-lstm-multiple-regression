...
for tmp_file_name in file_name:
    ...
    plt.savefig('./fig_res/lstm_'+tmp_file_name+'.png')
    plt.close('all')
    ...
    error_df.to_csv('./error_value/lstm_'+tmp_file_name+'.csv',index=False)
    ...
    pred_value_df.to_csv('./pred_value/'+tmp_file_name+'_lstm.csv',index=False)
